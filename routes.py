...
import os
from display import allRooms, allFavoriteRooms, addFavorite, removeFavorite, getFavoriteRooms, db_session, \
    getUserGroups, addNewGroup, deleteGroup, editGroupInfo, addRoomToGroup, removeRoomFromGroup, getUserGroupsJSON, clearRoomStatuses
...
def queryRooms():
    ...
    data = allRooms(username, college, firstranking,
                    lastranking, occupancy, building, year)
    roomids = []
        ...
        for room in data:
            if room.Room.room_id not in seen:
                mean = sum(rankings[room.Room.room_id]) / \
                    len(rankings[room.Room.room_id])
...
def favorites():
    ...
    fav_headings = ("BUILDING", "ROOM NO.", "OCCUPANCY",
                    "SQ. FOOTAGE", "FAVORITE")
...
def map():
    ...
    dorms = ["Butler College", "Forbes College", "Mathey College",
             "Rockefeller College", "Whitman College", "Wilson College", "Upperclass"]
    for i in dorms:
    ...
    for building in buildingrank:
        allrank[building] = sum(buildingrank[building]) / \
            len(buildingrank[building])
        maxrank[building] = max(buildingrank[building])
        minrank[building] = min(buildingrank[building])

    return make_response(render_template("map.html", allrank=allrank, maxrank=maxrank, minrank=minrank))
...
def groups():
    ...
    return make_response(render_template('groups.html', has_groups=has_groups, groups=groups))
...
def addroomtogroup():
    ...
    addRoomToGroup(username, json.loads(request.get_data())['room_id'], json.loads(request.get_data())['group_id'])
...
def removeroomfromgroup():
    ...
    # ajax to add favorite
    if request.method == 'POST':
        removeRoomFromGroup(username, json.loads(request.get_data())[
                            'room_id'], json.loads(request.get_data())['group_id'])
    else:
...
def getGroupsJSON():
    ...
    if len(groups) == 0:
        has_groups = False
    return jsonify({"groups": groups, "has_groups": has_groups, "username":username})
